...
for i_c=1:n_chan
    ...
    [SE_ts(i_c).mean,SE_ts(i_c).sd] = get_trial_means(SE_tfr(i_c).trials.TrialSummary);
...
%% helper functions
function [means,sds] = get_trial_means(TS)
feats{1} = {'meaneventpower','eventnumber','meaneventduration',     ...
            'meaneventFspan'};
feats{2} = {'power','rate','duration','Fspan'}; n_f = length(feats{1});
