function [TrialSummary,Events,IEI] = spectralevents_LL_find(findMethod, ...
                           eventBand, thrFOM, tVec, fVec, TFR, classLabels)
% SPECTRALEVENTS_FIND Algorithm for finding and calculating spectral 
%   events on a trial-by-trial basis of a single subject/session. 
%   Modified for Levin Lab by Gerardo Parra. Last updated 2023-03-29
%
%   Uses one of three methods before further analyzing and organizing event 
%   features:
...
spectralEvents_columnlabel={'trialind', 'classLabels', 'maximafreq', 'lowerboundFspan', 'upperboundFspan', 'Fspan', ...
    'tfr_maximatiming', 'tfr_onset', 'tfr_offset', 'tfr_duration', 'maximapower', 'maximapowerFOM'};
...
for tri=1:numTrials
  ...
  if nnz(spectralEvents(:,1)==tri)==0
    ...
    trialSummary.meaneventduration(tri) = mean(spectralEvents(spectralEvents(:,eventsind.trialind)==tri,eventsind.tfr_duration));
    trialSummary.meaneventFspan(tri) = mean(spectralEvents(spectralEvents(:,eventsind.trialind)==tri,eventsind.Fspan));
    trialSummary.mostrecenteventtiming(tri) = spectralEvents(find(spectralEvents(:,eventsind.trialind)==tri,1,'last'), eventsind.tfr_maximatiming);
    ...
    trialSummary.mostrecenteventduration(tri) = spectralEvents(find(spectralEvents(:,eventsind.trialind)==tri,1,'last'), eventsind.tfr_duration);
...
% convert trialSummary to more table-like struct
trialSummary = struct2struct_array(trialSummary);

% Inter-event interval (IEI)
ieitemp=diff(spectralEvents(:,eventsind.tfr_maximatiming));
...
for cls_i=1:numel(classes)
    ...
    iei_class=diff(spectralEvents(spectralEvents(:,eventsind.classLabels)==classes(cls_i),eventsind.tfr_maximatiming));
...
TrialSummary = struct('NumTrials',numTrials,'SpecialFeatures',specialFeat,'TrialSummary',trialSummary);
spectralEvents = num2cell(spectralEvents);
Events = struct('EventBand',eventBand,'ThrFOM',thrFOM,'MedianPower',medianpower,'Threshold',thr,'Events',struct(spectralEvents_columnlabel{1},spectralEvents(:,1),...
...
%% anonymous event-finding functions
