function [s_mean, s_var, s_all] = subj_avg_spec_events( sorted_data,    ...
                                                      feats, conds_to_inc )
...
%   Outputs:
%     - s_mean - subject averages of input features, sorted by group
%     - s_var  - subject stdevs of input features, sorted by group
%     - s_all  - all subject features, sorted by group
%
%%
    ...
    for i_g = 1:n_grps
        ...
        for i_s = 1:n_subj
            ...
            if i_s == 1
                ...
                s_all(i_c).(group).id  = {};
                for i_f = 1:n_feats
                    s_mean(i_c).(group).(feats{i_f}) = [];
                    s_var(i_c).(group).(feats{i_f})  = [];
                    s_all(i_c).(group).(feats{i_f})  = {};
                end
            end
            s_mean(i_c).(group).id{end+1,1} = subj(2:end);
            s_var(i_c).(group).id{end+1,1}  = subj(2:end);
            s_all(i_c).(group).id{end+1,1}  = subj(2:end);
            ...
            for i_f = 1:n_feats
                ...
                if isequal(feat,'eventnumber')
                    ...
                    if isempty(subj_data.TrialSummary)
                        s_mean(i_c).(group)(end+1,1) = NaN;
                        s_var(i_c).(group)(end+1,1)  = NaN;
                        s_all(i_c).(group)(end+1,1)  = NaN;
                        continue
                    end
                    % get trial features
                    ...
                    % save mean & stdev
                    s_mean(i_c).(group).(feat)(end+1,1) = feat_mean;
                    s_var(i_c).(group).(feat)(end+1,1)  = feat_std;
                    s_all(i_c).(group).(feat){end+1,1}  = trial_feats;  
                ...
                % event-level features
                    ...
                    s_mean(i_c).(group).(feat)(end+1,1) = feat_mean;
                    s_var(i_c).(group).(feat)(end+1,1)  = feat_std;
                    s_all(i_c).(group).(feat){end+1,1}  = event_feats;  
...
if n_chans > 1
    for i_g = 1:n_grps
        group = groups{i_g};
        chan_mean.(group).id = s_mean(1).(group).id;
        chan_var.(group).id  = s_var(1).(group).id;
        for i_f = 1:n_feats
            feat = feats{i_f};
            % get feature means/sds and average across channels
            group_means = [s_mean(:).(group)];
            group_means = mean([group_means(:).(feat)],2,'omitnan');
            group_vars  = [s_var(:).(group)];
