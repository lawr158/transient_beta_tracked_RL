...
for subj_j=1:numSubj
    ...
    for trl=1:size(X.(subj_id),2)
        [TFR_trl,tVec,~] = spectralevents_ts2tfr(X.(subj_id)(:,trl),fVec,Fs,7); %Transformation calculated using a Morlet wavelet (width=7), see 4DToolbox for function details)
