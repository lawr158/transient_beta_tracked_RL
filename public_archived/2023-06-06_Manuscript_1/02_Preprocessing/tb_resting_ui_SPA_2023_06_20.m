...
grp_proc_info.src_dir={'X:\Groups\SPA\01_Data_Raw_Summary_Processed\EEG\Participant_Data\03_Processed_Data\02_Baseline'}; %the directory containing your source files
grp_proc_info.beapp_curr_run_tag = 'tb-manuscript1_2023-06-21'; % The tag you would like to append to folder names for this run. def = '' or 'NONE'. 'NONE' mutes timestamping. If not given on a rerun, a timestamp will be used. 
grp_proc_info.beapp_prev_run_tag = 'tb-manuscript1_2023-06-21'; % def = ''.  run tag for previous run that you would like to use as source data for rerun. can be timestamp, but must be exact.
...
s1_left = [29 35 41 42 37 30]; s1_right = [105 111 110 103 93 87];
frontal = [23 19 16 4 3 1];    posterior = [60 67 72 77 85];
extra_channels = [s1_left s1_right frontal posterior];
grp_proc_info.beapp_ica_additional_chans_lbls{1} = extra_channels;
