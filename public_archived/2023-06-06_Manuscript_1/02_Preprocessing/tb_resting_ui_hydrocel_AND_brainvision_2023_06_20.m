...
grp_proc_info.src_dir={'L:\Private\ch226126\Transient_Beta\data\PHTS'}; %the directory containing your source files
grp_proc_info.beapp_curr_run_tag = 'tb-manuscript1_2023-06-23'; % The tag you would like to append to folder names for this run. def = '' or 'NONE'. 'NONE' mutes timestamping. If not given on a rerun, a timestamp will be used. 
grp_proc_info.beapp_prev_run_tag = 'tb-manuscript1_2023-06-23'; % def = ''.  run tag for previous run that you would like to use as source data for rerun. can be timestamp, but must be exact.
...
%
grp_proc_info.beapp_toggle_mods{'HAPPE_V3',{'Module_On','Module_Export_On'}}= [0,0]; % Had to include to turn "off" HAPEE v3, in line 68 of "prepare_to_run_main", Happee v3 reproccess has been hardcoded to 0, KS 6/23/23
%
...
grp_proc_info.src_presentation_software = 2; % presentation software used for paradigm (1 = EPrime, 2 = Presentation. def = 1)
...
grp_proc_info.src_unique_nets = {'HydroCel GSN 128 1.0','Brain Vision 64ch'}; % def ={''} If not running HAPP-E with multiple nets, optional for speed. Required for more than one net if running HAPP-E
...
grp_proc_info.beapp_filters{'Highpass','Filt_Cutoff_Freq'} = 0.1; % def = 1
...
grp_proc_info.beapp_ica_additional_chans_lbls{1} = [29,35,41,42,37,30,105,111,110,103,93,87,23,19,16,4,3,1,60,67,72,77,85];  % def = []. ex. [4,8,19,20]
grp_proc_info.beapp_ica_additional_chans_lbls{2} = [39,43, 40, 41,59, 55, 58, 57,34, 36, 35, 63, 62,44, 47, 49, 50, 53];
...
grp_proc_info.beapp_event_code_onset_strs = {'D255'}; %Ex {'stm+'} the event codes assigned during data collection to signifiy the onset of the stimulus. 
...
grp_proc_info.beapp_event_eprime_values.condition_names = {'D255'};
...
grp_proc_info.evt_seg_win_end   = 5;  % def = .800; end time in seconds for segments, relative to the event marker of interest (ex .800, 1) 
...
grp_proc_info.evt_analysis_win_end   = 5;  % def = .800; end time in seconds for analysis segments, relative to the event marker of interest (ex .800, 1) 
...
grp_proc_info.evt_trial_baseline_win_start = -0.5; % def = -0.100;  start time in seconds for baseline, relative to the event marker of interest (ex -0.100, 0). Must be within range you've segmented on. 
grp_proc_info.evt_trial_baseline_win_end   = 0; % def = -0.100;  start time in seconds for baseline, relative to the event marker of interest (ex -0.100, 0) 
...
grp_proc_info.win_select_first_n  = [];
...
grp_proc_info.bw(6,1:2)  = [56,100]; %bandwidth 8
grp_proc_info.bw_name(6) = {'HighGamma'}; %name of bandwidth 5
...
grp_proc_info.beapp_itpc_params.set_freq_range = 1;
grp_proc_info.beapp_itpc_params.min_freq = 6;
grp_proc_info.beapp_itpc_params.max_freq = 65;
grp_proc_info.beapp_itpc_params.min_cyc = 3;
grp_proc_info.beapp_itpc_params.max_cyc = 0.5;
