...
if ~isfield(C.path,'in'), C.path.in = C.path.beapp_files; end
if ~isfield(C.path,'out'), C.path.out = ''; end
if ~isfield(C,'tag'), C.tag = C.path.beapp_tag; end
...
% get length of segments in seconds
C.t_win.n_sec = abs(sum(C.t_win.to_keep))/1000;
